# hierarchical MNL fit with PMwG to exp1aDead
rm(list=ls())
...
## Name the parameters:
par_names <- c("u_cost_5_10", "u_sides_5_10", "u_deliveryTime_5_10",
...
# Guy and Scott's suggestions - email 9th April 2021# Mean centre attributes
for(i in 1:3) {
...
# log density of the observed choices under subject-level parameters (MNL, Null)
ll <- function(x, data, sample = FALSE) {
  ...
  u[deadlines,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][deadlines,] * x["u_cost_5_10"]) +
      (data[paste0("opt", 1:n.options, "mcSides")][deadlines,] * x["u_sides_5_10"]) +
      (data[paste0("opt", 1:n.options, "mcDeliveryTime")][deadlines,] * x["u_deliveryTime_5_10"]))
  u[!deadlines,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][!deadlines,] * x["u_cost_20"]) +
      (data[paste0("opt", 1:n.options, "mcSides")][!deadlines,] * x["u_sides_20"]) +
      (data[paste0("opt", 1:n.options, "mcDeliveryTime")][!deadlines,] * x["u_deliveryTime_20"]))
