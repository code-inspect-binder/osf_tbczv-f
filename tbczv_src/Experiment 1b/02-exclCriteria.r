# ********** Start of header **************
# Title: Exp1b Mobile Phone Deadline: Exclusion criteria
#
...
dim(complete.subj)
# store subjs with 10 blocks from complete.subj 
keepers <- names(complete.subj)[complete.subj == 8]
# Keep subjs with 12 blocks 
new.data <- data[as.character(data$subj) %in% keepers,]
# Check all subjs have completed 8 blocks
table(new.data$subj)
# Drop levels of subjs with 0 (Subjs removed w/ < 8 blocks)
...
# Total number of missed trials = 354
# Number missed trials on 5 second deadline = 339 trials
nrow(data[data$maxTrialTime==5 & is.na(data$response),])
# Number missed trials on 10 second deadline = 13 trials
nrow(data[data$maxTrialTime==10 & is.na(data$response),])
# Number missed trials on 20 second deadline = 2 trials
nrow(data[data$maxTrialTime==20 & is.na(data$response),])
...
# TRDM dataset - all trials remain in the data set and their likelihood is according to the tail probability
...
data <- data[!data$trialError == " Slow",] 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#