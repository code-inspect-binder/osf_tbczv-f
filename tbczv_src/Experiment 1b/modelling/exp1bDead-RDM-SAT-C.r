# hierarchical RDM fit with PMwG to exp1bDead
rm(list=ls())
...
par_names <- c("u_cost_5", "u_camera_5", "u_memory_5", "u_battery_5",
               "u_cost_10_20", "u_camera_10_20", "u_memory_10_20", "u_battery_10_20",
...
# log density of the observed choices under subject-level parameters (RDM, NSAT-C)
ll <- function(x, data, sample=FALSE) {
  ...
  deadlines <- data$maxTrialTime == 5
  u <- drifts <- array(dim=c(nrow(data), n.options), data=NA)
  
  u[deadlines,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][deadlines,] * x["u_cost_5"]) +
      (data[paste0("opt", 1:n.options, "mcCamera")][deadlines,] * x["u_camera_5"]) +
      (data[paste0("opt", 1:n.options, "mcMemory")][deadlines,] * x["u_memory_5"]) +
      (data[paste0("opt", 1:n.options, "mcBattery")][deadlines,] * x["u_battery_5"]))  
  
  u[!deadlines,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][!deadlines,] * x["u_cost_10_20"]) +
      (data[paste0("opt", 1:n.options, "mcCamera")][!deadlines,] * x["u_camera_10_20"]) +
      (data[paste0("opt", 1:n.options, "mcMemory")][!deadlines,] * x["u_memory_10_20"]) +
      (data[paste0("opt", 1:n.options, "mcBattery")][!deadlines,] * x["u_battery_10_20"]))
