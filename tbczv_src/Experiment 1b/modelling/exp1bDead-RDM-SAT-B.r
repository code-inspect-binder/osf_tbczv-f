# hierarchical RDM fit with PMwG to exp1bDead-SAT-B
rm(list=ls())
...
## Name the parameters:
par_names <- c("u_cost_5_10", "u_camera_5_10", "u_memory_5_10", "u_battery_5_10",
...
# These changes applied to data set post exclusion criteria script 
#   i.e. changes included in exp1aDeadRDM.RData object
# for(i in 1:3) {
...
# log density of the observed choices under subject-level parameters (RDM, SAT-B)
ll <- function(x, data, sample=FALSE) {
  ...
  deadlines <- data$maxTrialTime == 5 | data$maxTrialTime == 10
  u <- drifts <- array(dim=c(nrow(data), n.options), data=NA)
  
  u[deadlines,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][deadlines,] * x["u_cost_5_10"]) +
      (data[paste0("opt", 1:n.options, "mcCamera")][deadlines,] * x["u_camera_5_10"]) +
      (data[paste0("opt", 1:n.options, "mcMemory")][deadlines,] * x["u_memory_5_10"]) +
      (data[paste0("opt", 1:n.options, "mcBattery")][deadlines,] * x["u_battery_5_10"]))  
  
  u[!deadlines,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][!deadlines,] * x["u_cost_20"]) +
      (data[paste0("opt", 1:n.options, "mcCamera")][!deadlines,] * x["u_camera_20"]) +
      (data[paste0("opt", 1:n.options, "mcMemory")][!deadlines,] * x["u_memory_20"]) +
      (data[paste0("opt", 1:n.options, "mcBattery")][!deadlines,] * x["u_battery_20"]))
  
  # drift rates are exponent of utilities (Hawkins et al. Integrating cog processes, 2014)
  drifts[deadlines,] <- exp(u[deadlines,])
  drifts[!deadlines,] <- exp(u[!deadlines,])
...
## Some priors.
priors <- list(
  theta_mu_mean = c(rep(0, 8), 2, 2, 2,0),
