# hierarchical RDM fit with PMwG to exp2bSAT
rm(list=ls())
...
par_names <- c("u_cost_speed", "u_memory_speed", "u_camera_speed", "u_battery_speed",
               "u_cost_accuracy", "u_memory_accuracy", "u_camera_accuracy", "u_battery_accuracy",
...
data <- subset(data, trialError == "FALSE")
...
# log density of the observed choices under subject-level parameters (RDM, SAT)
ll <- function(x, data, sample=FALSE) {
  ...
  u[is.speed,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][is.speed,] * x["u_cost_speed"]) +
      (data[paste0("opt", 1:n.options, "mcMemory")][is.speed,] * x["u_memory_speed"]) +
      (data[paste0("opt", 1:n.options, "mcCamera")][is.speed,] * x["u_camera_speed"]) +
      (data[paste0("opt", 1:n.options, "mcBattery")][is.speed,] * x["u_battery_speed"]))
  
  u[!is.speed,] <- as.matrix(
    (data[paste0("opt", 1:n.options, "mcCost")][!is.speed,] * x["u_cost_accuracy"]) +
      (data[paste0("opt", 1:n.options, "mcMemory")][!is.speed,] * x["u_memory_accuracy"]) +
      (data[paste0("opt", 1:n.options, "mcCamera")][!is.speed,] * x["u_camera_accuracy"]) +
      (data[paste0("opt", 1:n.options, "mcBattery")][!is.speed,] * x["u_battery_accuracy"]))
...
start_points <- list(
  mu = c(-.02, .025, .01, .02, -.02, .025, .01, .02,
         log(c(5, 7, .2))),
